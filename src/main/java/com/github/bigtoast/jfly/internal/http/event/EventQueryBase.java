...
import com.github.bigtoast.jfly.api.event.EventField;
...
abstract public class EventQueryBase<T extends JFlyQuery<T,Event>> extends JFlyQueryBase<T,Event> {
	...
	public String build() {
		...
		try {	
			...
			if ( hasFromDate() ){
				str.append("&fromDate=");
				str.append(URLEncoder.encode(JFlyUtils.toDateString(fromDate), encoding));
			}
			if ( hasThroughDate() ){
